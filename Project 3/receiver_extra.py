...
def main():
    ...
    with open(args.out_file, 'wb') as write_file:
        RCV_NXT = 0
        hand_shake = client_sock.recv(512)
        handshake_info = make_TCP_UNPACK(hand_shake)
        RCV_NXT = int(handshake_info['sequence_number']) + 1
        header = make_TCP_PACK(ISN_RECV, RCV_NXT, SYN=1, ACK=1)
        client_sock.sendto(header, server_addr)
        # try
        RCV_NXT = 0
        ISN_RECV = 0
